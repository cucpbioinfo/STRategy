...
import th.ac.chula.fims.payload.dto.MotifDto;
...
public class ConfigurationServiceImpl implements ConfigurationService {
    ...
    public MessageResponse updateReferencePatterns(MultipartFile file) throws IOException {
        ...
        for (int i = 0; i < numberOfSheets; i++) {
            ...
            for (int j = 1; j <= sheet.getLastRowNum(); j++) {
                ...
                Boolean isReverse = resolveIsReverse(data.get(3));
                ...
                List<PatternAlignmentMotif> motifDtoList = SequenceUtils.convertSeqStringToMotif(sequence);
    ...
    private static Boolean resolveIsReverse(String data) {
        if (data.equalsIgnoreCase("true")) {
            return true;
        } else if (data.equalsIgnoreCase("false")) {
            return false;
        }
        throw new BadFormatException("isReverse's value is invalid");
    }
